...
class Stat {
    ...
    static async _timeout(pid, delay) {
        ...
        if (delay >= config.timeout) await redis.incr(KEY.TIMEOUT(pid, date))
    ...
    static async day(pid, date) {
        ...
        let request_updatetime = await redis.get(
            KEY.REQUEST_UPDATETIME(pid, date)
        )
    ...
    static async requests(size) {
        ...
        try {
            ...
            for (var i = 0; i < list.length; i++) {
                ...
                if (
                    requests[i].ip &&
                    Array.isArray(requests[i].ip) &&
                    requests[i].ip.length
                ) {
                    for (var j = 0; j < requests[i].ip.length; j++) {
                        requests[i].ip[j] = requests[i].ip[j].replace(
                            /^(\d*)\.(\d*)/,
                            '***.***'
                        )
                    }
                } else if (requests[i].ip) {
                    requests[i].ip = requests[i].ip.replace(
                        /^(\d*)\.(\d*)/,
                        '***.***'
                    )
                }
