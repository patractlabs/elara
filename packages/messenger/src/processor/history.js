...
class History {
    ...
    async process(message) {
        let replacement_id = Buffer.from(crypto.randomBytes(16)).readUIntLE(
            0,
            4
        )
        ...
        try {
            switch (message.request.method) {
                case 'chain_getBlockHash':
                    return await this._chainGetBlockHash(
                        message.request,
                        replacement_id
                    )

                case 'state_getStorage':
                    return await this._stateGetStorage(
                        message.request,
                        replacement_id
                    )

                case 'state_queryStorageAt':
                    return await this._stateQueryStorageAt(
                        message.request,
                        replacement_id
                    )
        ...
        */
        try {
            ...
            let key_maxnum = await this.storage.findAll({
                attributes: [
                    'key',
                    [Sequelize.fn('max', Sequelize.col('block_num')), 'maxnum'],
                ],
                where: where,
                group: ['key'],
            })
                        ...
                        [Op.and]: [
                            ...
                            },
                            {
                                key: {
                                    [Op.eq]:
                                        '\\x' +
                                        Buffer.from(key_maxnum[i].key).toString(
                                            'hex'
                                        ),
                                },
                            },
                ...
                this.storage
                    ...
                    .then((data) => {
                        if (this.replacement_msg[replacement_id]) {
                            ...
                            for (let i = 0; i < request.params[0].length; i++) {
                                ...
                                for (let j = 0; j < data.length; j++) {
                                    if (
                                        k ==
                                        '0x' +
                                            Buffer.from(data[j].key).toString(
                                                'hex'
                                            )
                                    ) {
                            ...
                            message.id = this.replacement_msg[
                                replacement_id
                            ].request.id
                            let id = this.replacement_msg[replacement_id].id
                            let chain = this.replacement_msg[replacement_id]
                                .chain
                ...
                .then((data) => {
                    if (data && this.replacement_msg[replacement_id]) {
                        ...
                        if (null != data.storage) {
                            result =
                                '0x' + Buffer.from(data.storage).toString('hex')
                        }
                        let message = {
                            jsonrpc: '2.0',
                            result: result,
